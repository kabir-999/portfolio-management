...
const GhostCursor = ({
  ...
  const fragmentShader = `
    ...
    vec4 blob(vec2 p, vec2 mousePos, float intensity, float activity) {
      ...
      float smoke = fbm(p * iScale * 3.0 + r * 0.8);
      float distFactor = 1.0 - smoothstep(0.0, iRadius * activity, length(p - mousePos));
      float alpha = pow(smoke, 3.5) * distFactor;
  ...
  useEffect(() => {
    ...
    resize();
    const handleResize = () => { if (active) resize(); };
    window.addEventListener('resize', handleResize);
    resizeObsRef.current = { disconnect: () => window.removeEventListener('resize', handleResize) };
    ...
    const onPointerMove = e => {
      const x = THREE.MathUtils.clamp(e.clientX / Math.max(1, window.innerWidth), 0, 1);
      const y = THREE.MathUtils.clamp(1 - e.clientY / Math.max(1, window.innerHeight), 0, 1);
  ...
  useEffect(() => {
    if (materialRef.current) {
      materialRef.current.uniforms.iRadius.value = radius;
    }
  }, [radius]);
